...
public class SymbolBuilder {
    ...
    private var configuration: NSImage.SymbolConfiguration
    ...
    public func font(_ fontSize: CGFloat, weight: NSFont.Weight) -> Self {
        configuration = configuration.applying(.init(pointSize: fontSize, weight: weight))
        return self
    }

    public func hierarchicalColor(_ color: NSColor) -> Self {
        configuration = configuration.applying(.init(hierarchicalColor: color))
        return self
    }

    public func paletteColors(_ colors: [NSColor]) -> Self {
        configuration = configuration.applying(.init(paletteColors: colors))
        return self
    }

    public func scale(_ scale: NSImage.SymbolScale) -> Self {
        configuration = configuration.applying(.init(scale: scale))
        return self
    }

    public func build() -> NSImage {
        guard let image = NSImage(systemSymbolName: name.rawValue, accessibilityDescription: nil)?.withSymbolConfiguration(configuration) else {
